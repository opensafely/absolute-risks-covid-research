********************************************************************************
*
*	Do-file:		AL012_cox_regression_CC_BMI.do
*
...
*	Data created:	None
*
*	Other output:	Log file:  logs/AL012_cox_regression_CC_BMI_wave`i'_`out'_`exp'.log
*					Estimates:	output/
*									ldcox_wave`i'_`out'_`exp'_CC_BMI.out
*
* 						i = Wave (1 or 2)
...
postfile `ldrresults' 	wave str15(outcome) str15(exposure) str20(model)	///
	...
	* Complete case for BMI
	...
	strate `exp', 														///
			output(output/data_temp`i'_`out'_`exp'_CC_BMI, replace) 	///
			per(10000)
...
* Save data
save "output/ldhrs_wave`i'_`out'_`exp'_CC_BMI", replace
...
use "output/data_temp`i'_`out'_`exp'_CC_BMI", clear
gen wave = `i'
gen exp = "`exp'"
gen out = "`out'"
erase "output/data_temp`i'_`out'_`exp'_CC_BMI.dta"
...
* Save data
save "output/ldrates_wave`i'_`out'_`exp'_CC_BMI", replace
...
use "output/ldrates_wave`i'_`out'_`exp'_CC_BMI.dta", clear
merge 1:1 outcome exposure expcat using ///
	"output/ldhrs_wave`i'_`out'_`exp'_CC_BMI.dta", assert(match) nogen
order wave outcome exposure expcat
outsheet using "output/ldcox_wave`i'_`out'_`exp'_CC_BMI.out", replace
erase "output/ldrates_wave`i'_`out'_`exp'_CC_BMI.dta"
erase "output/ldhrs_wave`i'_`out'_`exp'_CC_BMI.dta"
