********************************************************************************
*
*	Do-file:		AL006b_cox_regression_interactions_sex.do
*
...
*	Data created:	None
*
*	Other output:	Log file:  logs/AL006a_cox_regression_inter_sex_wave`i'_`out'_`exp'.log
*					Estimates:	output/
*									ldcox_inter_sex_wave`i'_`out'_`exp'.out
*
* 						i = Wave (1 or 2)
...
*					exploring interactions with broad age strata.
*  
********************************************************************************
...
* Open a log file
cap log close
log using "logs/AL006b_cox_regression_inter_sex_wave`i'_`out'_`exp'", replace t
...
postfile `ldrresults' 	wave str15(outcome) str15(exposure) str20(model)	///
						expcat male lnhr sehr using `ldrfile'
	...
	* Confounder only model
	stcox i.`exp'##i.male age1 age2 age3 i.ethnicity_5, 	///
	...
	forvalues k = `lo_`exp'' (1) `hi_`exp'' {
		...
		if _rc==0 {
			post `ldrresults' (`i') ("`out'") ("`exp'")		///
				("Confounders") 							///
				(`k') (0) (_b[`k'.`exp']) (_se[`k'.`exp'])
		}
		forvalues l = 1 (1) 1 {
			capture qui di _b[`k'.`exp'#`l'.male]
			if _rc==0 {
			    lincom `k'.`exp' + `k'.`exp'#`l'.male
				post `ldrresults' (`i') ("`out'") ("`exp'")		///
					("Confounders") 							///
					(`k') (`l') (r(estimate)) (r(se))
			}
	...
	* All variables
	stcox i.`exp'##i.male age1 age2 age3 	 			///
				i.ethnicity_5 i.imd resid_care_ldr 		///
				obese40 								///
...
* Put in wide format
reshape wide hr_ci, i(wave outcome exposure expcat male) j(adjust)
rename hr_ci1 hr_conf
rename hr_ci2 hr_all


order wave outcome exposure male category hr*
sort wave outcome exposure male expcat

* Save data
outsheet using "output/ldcox_inter_sex_wave`i'_`out'_`exp'", replace
