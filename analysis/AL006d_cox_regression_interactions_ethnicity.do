********************************************************************************
*
*	Do-file:		AL006d_cox_regression_interactions_ethnicity.do
*
...
*	Data created:	None
*
*	Other output:	Log file:  logs/AL006a_cox_regression_inter_ethnicity_wave`i'_`out'_`exp'.log
*					Estimates:	output/
...
*					exploring interactions with ethnicity.
*  
********************************************************************************
...
* Open a log file
cap log close
log using "logs/AL006d_cox_regression_inter_ethnicity_wave`i'_`out'_`exp'", replace t
...
postfile `ldrresults' 	wave str15(outcome) str15(exposure) str20(model)	///
						expcat ethnicity_5 lnhr sehr using `ldrfile'
	...
	* Confounder only model
	stcox i.`exp'##i.ethnicity_5 age1 age2 age3 male, 	///
	...
	forvalues k = `lo_`exp'' (1) `hi_`exp'' {
		...
		forvalues l = 2 (1) 5 {
			capture qui di _b[`k'.`exp'#`l'.ethnicity_5]
			if _rc==0 {
			    lincom `k'.`exp' + `k'.`exp'#`l'.ethnicity_5
				post `ldrresults' (`i') ("`out'") ("`exp'")		///
					("Confounders") 							///
					(`k') (`l') (r(estimate)) (r(se))
			}
...
* Put in wide format
reshape wide hr_ci, i(wave outcome exposure expcat ethnicity_5) j(adjust)
rename hr_ci1 hr_conf
rename hr_ci2 hr_all

order wave outcome exposure ethnicity_5 category hr*
sort wave outcome exposure ethnicity_5 expcat

* Save data
outsheet using "output/ldcox_inter_ethnicity_wave`i'_`out'_`exp'", replace
